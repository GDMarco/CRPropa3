...
double TabularPhotonField::getPhotonDensity(const double ePhoton, double z, const Vector3d &pos) const {
	if (this->isRedshiftDependent) {
...
TabularSpatialPhotonField::TabularSpatialPhotonField(std::string fieldName, bool isRedshiftDependent, bool isSpatialDependent) {
    ...
    if (this->isRedshiftDependent) {
        ...
        for (auto const& dir_entry : std::__fs::filesystem::directory_iterator{dirE}) {
            ...
            std::vector<double> vE = readPhotonEnergy(dir_entry.path().string()); // filename: + this->FieldName + pos + "_photonEnergy.txt");
            
            this->photonEnergies.push_back(vE);
            
        }
        
        this->photonDict = photonDict; //check
...
double BlackbodyPhotonField::getPhotonDensity(const double ePhoton, double z, const Vector3d &pos) const {
