...
EMPairProduction::EMPairProduction(ref_ptr<PhotonField> photonField, bool haveElectrons, double thinning, double limit) { //, ref_ptr<InteractionRates> interactionRates
    setPhotonField(photonField); //, interactionRates
	setThinning(thinning);
	setLimit(limit);
	setHaveElectrons(haveElectrons);
}

void EMPairProduction::setPhotonField(ref_ptr<PhotonField> photonField) { //interactionRates
    ...
    if (!this->photonField->hasSpatialDependence()){
        ...
        initRate(getDataPath("EMPairProduction/rate_" + fname + ".txt"), intRatesIso); //intRatesIso
...
void EMPairProduction::performInteraction(Candidate *candidate) const {
    ...
    std::cout << "here perform" << std::endl;
...
void EMPairProduction::process(Candidate *candidate) const {
    ...
    std::cout << "here process" << std::endl;
    
    getProcessTabs(position, tabEnergy, tabRate);
    std::cout << "tabEnergy size" << tabEnergy.size() << std::endl;
    std::cout << "tabRate size" << tabRate.size() << std::endl;
    std::cout << "here process tabs end" << std::endl;
    
    // check if in tabulated energy range
    if ((E < tabEnergy.front()) or (E > tabEnergy.back())) {
        std::cout << "here process if " << std::endl;
        return;}
    
    std::cout << "here process end if" << std::endl;
        
    // interaction rate
    double rate = interpolate(E, tabEnergy, tabRate);
    std::cout << "here process interpolate rate" << std::endl;
    
    rate *= pow_integer<2>(1 + z) * photonField->getRedshiftScaling(z);
    
    std::cout << "here process redshift scaling " << std::endl;
